...
// Captured once in beforeAll from one genuine Admin sign-in, then reused
// in-memory (never written to disk — see the adminPage fixture below) by
// every test that only needs an already-authenticated Admin session as a
// precondition. Manual-acceptance correction: this suite previously called
// loginAsAdmin() — a real sign-in — 18 times per run against one shared
// fixture email, comfortably exceeding the app's own 5-attempts-per-15-
// minutes login rate limit (lib/auth/actions.ts's LOGIN_RATE_LIMIT) partway
// through the file. Reusing one captured session for the 17 tests that
// don't test login itself brings that down to 2 real Admin sign-ins per
// run (this capture, plus "Admin is allowed to manage students", which
// deliberately keeps a fresh login since verifying that IS what it tests).
...
// Extended test object providing `adminPage`: a page in its own fresh,
// isolated BrowserContext (closed after each test, same isolation
// guarantee Playwright's default `page` fixture already gives every test),
// seeded with the captured Admin session above instead of starting
// unauthenticated. Never shared across roles — nothing here ever touches
// the trainer/student/super-admin credentials, and never shared across
// runs — `adminStorageState` is a plain in-memory value scoped to this
// module instance, recreated fresh every run and discarded when the
// process exits.
const adminTest = test.extend<{ adminPage: Page }>({
  // Playwright's own convention names this second parameter `use`, but
  // that collides with eslint-plugin-react-hooks's naming heuristic (it
  // flags any function literally called `use` as a React Hook) — renamed
  // here to the equally-clear `provideFixture`, no functional difference.
  adminPage: async ({ browser }, provideFixture) => {
    if (!adminStorageState) {
      throw new Error(
        "Admin storage state was not established — beforeAll must have failed.",
      );
    }
...
test.describe("Admin authentication-state reuse", () => {
  // Deliberately does no sign-in of its own — the entire point is to prove
  // the state captured once in beforeAll is what's granting access below,
  // not a fresh login this test happens to also perform. Every other
  // adminTest in this file relies on the same guarantee implicitly; this
  // one asserts it directly, and does so without consuming another attempt
  // against the login rate limiter.
  adminTest(
    "the shared adminPage fixture is a genuinely authenticated Admin session",
    async ({ adminPage: page }) => {
      ...
      expect(
        cookies.some((c) => c.name.startsWith("sb-")),
        "expected the reused Admin session to carry a persisted sb-* auth cookie",
      ).toBe(true);
    },
  );
});
...
test.describe("Add Student page: hydration and the country selector", () => {
  adminTest(
    "loads without a server/client hydration mismatch",
    async ({ adminPage: page }) => {
      ...
      const hydrationErrors = consoleErrors.filter((t) =>
        HYDRATION_ERROR_PATTERN.test(t),
      );
      expect(
        hydrationErrors,
  ...
  adminTest(
    "country selector supports all current libphonenumber-js countries, India first and default",
    async ({ adminPage: page }) => {
      await page.goto("/admin/students/new");
...
test.describe("Create students with international phone numbers", () => {
  adminTest(
    "create with an India phone normalizes and stores canonical E.164",
    async ({ adminPage: page }) => {
      await page.goto("/admin/students/new");
  ...
  adminTest(
    "create with a Canada phone normalizes and stores canonical E.164",
    async ({ adminPage: page }) => {
      await page.goto("/admin/students/new");
  ...
  adminTest(
    "create with a UK phone normalizes and stores canonical E.164",
    async ({ adminPage: page }) => {
      await page.goto("/admin/students/new");
  ...
  adminTest(
    "create with a UAE phone normalizes and stores canonical E.164",
    async ({ adminPage: page }) => {
      await page.goto("/admin/students/new");
  ...
  adminTest(
    "an invalid phone number is rejected with a visible field error, nothing created",
    async ({ adminPage: page }) => {
      await page.goto("/admin/students/new");
...
test.describe.serial("Duplicate detection and the override flow", () => {
  adminTest(
    "duplicate phone (different valid format) blocks creation, then the confirmed override with a reason succeeds",
    async ({ adminPage: page }) => {
      await page.goto("/admin/students/new");
  ...
  adminTest(
    "duplicate email (different phone) shows a warning naming the email match",
    async ({ adminPage: page }) => {
...
test.describe.serial("Profile management: edit, status, notes, documents", () => {
  ...
  adminTest(
    "create the student used for the rest of this flow",
    async ({ adminPage: page }) => {
      await page.goto("/admin/students/new");
  ...
  adminTest(
    "edit updates a profile field and the Student ID stays immutable",
    async ({ adminPage: page }) => {
  ...
  adminTest(
    "status can be changed and the new status persists",
    async ({ adminPage: page }) => {
      await page.goto(`/admin/students/${studentId}`);
  ...
  adminTest(
    "a staff note can be added and appears immediately",
    async ({ adminPage: page }) => {
      await page.goto(`/admin/students/${studentId}`);
  ...
  adminTest(
    "a document can be uploaded and then deleted",
    async ({ adminPage: page }) => {
      await page.goto(`/admin/students/${studentId}`);
...
test.describe("Student list: search, filter, pagination basics", () => {
  adminTest(
    "search finds a created student by name and by Student ID",
    async ({ adminPage: page }) => {
      await page.goto(`/admin/students?q=${encodeURIComponent(tag("IndiaA"))}`);
      await expect(
  ...
  adminTest(
    "status filter includes an active match and excludes an archived-only search",
    async ({ adminPage: page }) => {
      await page.goto(
  ...
  adminTest(
    "pagination controls reflect the current page and total when more than one page exists",
    async ({ adminPage: page }) => {
      ...
      await page.goto(
        `/admin/students?q=${encodeURIComponent(PHASE5_E2E_STUDENT_PREFIX)}`,
      );
